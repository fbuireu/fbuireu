...
- `github-activity.yml` invokes the same action four times, once per config under `.github/config/github-activity/`. Those configs are also where the per-locale wording of activity lines lives — `en.config.yml` carries no `messages:` block because English is the action's default.
