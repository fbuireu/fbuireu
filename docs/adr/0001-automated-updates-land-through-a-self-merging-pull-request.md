...
- **A generator that commits for itself makes this decision unenforceable, silently, and two of the three callers were such generators.** [`github-activity.yml`](../../.github/workflows/github-activity.yml) and [`wakatime-stats.yml`](../../.github/workflows/wakatime-stats.yml) called the composite action like the third and produced no Automated Update between them in their whole history: `Readme-Workflows/recent-activity` runs `git add / pull / commit / push` at the end of every run with no input that disables it, and `athul/waka-readme` commits and pushes as soon as it is given a `COMMIT_MESSAGE`, so the composite found `main` already carrying the change and reported `pull-request-operation = none`. The composite's own `continue-on-error: true` on its first attempt is what kept that invisible. Both steps have been removed rather than left standing as a claim the tree does not honour; both workflows are documented as pushing directly, and the three costs listed in the Context above are accepted for them. Only [`snake-animation.yml`](../../.github/workflows/snake-animation.yml) still routes through the composite, because `Platane/snk` writes files and commits nothing. The rule now governs one Refresh, and that is worth knowing before adding a sixth: it holds only for a generator that leaves the committing to us.
