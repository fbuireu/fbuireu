...
[`.github/actions/create-auto-merge-pr`](./.github/actions/create-auto-merge-pr) is the only first-party automation in the repository. Two Refreshes call it (`wakatime-stats.yml` and `snake-animation.yml`), and both pass `force-merge: 'true'`. Read [ADR 0001](./docs/adr/0001-automated-updates-land-through-a-self-merging-pull-request.md) before changing anything in it, and the Gotchas bullet on `github-activity.yml` for the one Refresh that does not route through it.
...
| `snake-grid-animation` | two contribution-grid SVGs, last written 2025-06-26 | **nothing**: dead leftover of the old `snk` push mode, referenced by nothing since the Editions were repointed at `dist/`. Safe to delete. |
...
- **`github-activity.yml` is the one Refresh that pushes straight to `main`, and it always was.** The action's `commitFile` runs `git add / pull / commit / push` unconditionally, with no flag to turn it off and nothing in the four config files that could; the four configs set only `commit_msg`, `readme_file` and the message strings. So by the time a `create-auto-merge-pr` step ran there was nothing left to propose: run 1724 logged *Branch 'activity-update-1724' is not ahead of base 'main' and will not be created* and `pull-request-operation = none`, on top of a `main` the same job had already moved. That step is gone rather than left as a silent no-op, which also takes the Owner Token out of every step in the workflow but the checkout, and `pull-requests: write` with it.
