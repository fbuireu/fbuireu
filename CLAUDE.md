...
- **The snake is quoted from `dist/` on `main`, and there is no longer a branch to quote it from.** `snake-grid-animation` held two contribution-grid SVGs last written on 2025-06-26 and was referenced by nothing from the day the Editions were repointed at `dist/`; it is deleted. This table used to carry a row for it saying it was safe to delete, which is a note, not a fix.
